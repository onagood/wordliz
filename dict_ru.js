...
const ROOT_DICT=['хуй','хуя','хуе','хуё','пизд','блядь','бляди','бляде','блядю','ебал','ебан','ебат','ебен','ебет',
  ...
  // added: these read as obscene to a player even when the wiki glosses them
  // politely, and no innocent Russian word contains them
  'жоп','говн','дерьм','срак','сран','шлюх','мраз','ублюд',
  // второй проход: анатомия, наркотики и брань, которых первый список не касался.
  // игра печатает определение к каждому «красному» слову, то есть выводит их
  // на экран сама, а не просто терпит в списке
  'пенис','сперм','вагин','дроч','шалав','курв','падл','сукин',
  'героин','кокаин','опиум','нацист','суицид','педоф','бордел','проститут'];
// Stems too dangerous to match as substrings: сук- also builds сукно and сук
// (a bough), хер- also builds херес and херик. These forms are dropped whole.
const WORD_DICT=new Set(['сука','суки','суке','суку','сукой',
  'хер','хера','херу','хером','херы',
  // «трах» сидит внутри «страх», «трахея», «метрах», «титрах», «шхерах» —
  // только целыми формами
  'трах','трахал','трахать','трахни','трахну','трахаю','трахая','трахай','трахнул',
  'трахает','трахают','трахали','трахаем']);
const hits=(w,roots)=>roots.some(r=>w.includes(r));
const crude=w=>hits(w,ROOT_DICT)||WORD_DICT.has(w);
...
const STOP_SEED=new Set([
  ...
  // настоящие слова, но просить игрока их собрать — не то же самое, что
  // случайно на них наткнуться: как «красные» они остаются в валидации
  'труп','аборт','водка','казнь','пытка','героин','кокаин',
  // имена собственные и прилагательные, пролезшие из списка лемм:
  // «дикая» и «синяя» — реки, «деда» и «диана» — имена, «всякая/всякое» —
  // краткие формы прилагательного, «берлин» — город
  'деда','диана','дикая','синяя','всякая','всякое','берлин',
  // «убийца» — обычное слово и в валидации остаётся, но загадывать его игра
  // не должна; «пенис» и «сперма» вычищены и из валидации фильтром выше
  'пенис','сперма','убийца'
]);
...
for(const [w,rk] of rank){
  ...
  const L=w.length;
  if(rk<=RANK_D&&!crude(w)) d[L].add(w);
...
for(const w of nouns){
  if(big.has(w)&&!crude(w)) d[w.length].add(w);
}
